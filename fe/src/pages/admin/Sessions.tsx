...
const Sessions = () => {
  ...
  const fetchSessions = async () => {
    try {
      const response = await fetch("/api/sessions?organizationId=YOUR_ORGANIZATION_ID");
      if (!response.ok) {
        throw new Error("Failed to fetch sessions");
      }
      const data = await response.json();
      refreshData(); // Update the context with fetched sessions
    } catch (error) {
  ...
  const handleCreateSession = async () => {
    try {
      ...
      refreshData(); // Update the context with the new session
  ...
  const handleDeleteSession = async (sessionId: string) => {
    try {
      const response = await fetch(`/api/sessions/${sessionId}?organizationId=YOUR_ORGANIZATION_ID`, {
        method: "DELETE",
      });
      ...
      refreshData(); // Update the context by removing the deleted session
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="bg-[#252525] text-white border-[#2F2F2F]">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button className="bg-[#B967FF] hover:bg-[#8344B8]" onClick={handleCreateSession}>
                  Create Session
                </Button>
              </div>
            </DialogContent>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {Array.isArray(session.skills) &&
                    session.skills.map((skill, index) => (
                      <span key={index} className="bg-[#8344B8] text-white text-xs px-2 py-1 rounded-full">
                        {skill}
                      </span>
                    ))}
                </div>
